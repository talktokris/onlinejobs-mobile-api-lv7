...
@section('title', 'Edit ' . $categoryName)
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-8">
        ...
        <div class="text-center mb-8">
            ...
            <h2 class="text-2xl font-semibold text-gray-800 mb-2">Edit {{ $categoryName }}</h2>
            <p class="text-sm text-gray-500">Update the {{ strtolower($categoryName) }} information</p>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('admin.settings.update', [$category, $item->id]) }}" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <select name="type" 
                        ...
                        @if(isset($distinctTypes) && count($distinctTypes) > 0)
                            @foreach($distinctTypes as $typeValue)
                                <option value="{{ $typeValue }}" {{ old('type', $item->type ?? '') == $typeValue ? 'selected' : '' }}>{{ $typeValue }}</option>
                            @endforeach
                        @endif
                    </select>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="status" 
                        ...
                        <option value="0" {{ old('status', $item->status ?? '1') == '0' ? 'selected' : '' }}>Not Active</option>
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
